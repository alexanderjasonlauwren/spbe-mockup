...
| distribution | mock + http | the printed route sheet only (no per-stop address/time on the wire). Scheduling approved orders onto a plan is real as of D3 B-Step 2 — `addApprovedOrders` calls `orderApi.http.ts`'s own `scheduleOneOrder`, not a second copy of the request. `dispatchTrip` (D3 A-Step 2/3, sending a confirmed trip's stops out for the first time — no prior call to `POST /distribution/trips/:id/dispatch` existed anywhere in the console) is real end to end, including the credit-limit override path, with two known gaps: the second-approver picker lists every active user in the tenant unfiltered, since `GET /users` returns no role/permission per user and no endpoint answers "who holds `distribution.override.deliveries`" — the request itself is what enforces it, an unauthorised choice comes back as its own clean refusal rather than the picker silently narrowing; and which outlet a breach names is resolved by matching the service's own message text against the trip's stop names, since the error carries no structured outlet id. The mock's own breach rule (`outletExposure`, already built for the plan-confirm-time block) is reused as-is rather than reconciled with the real one: it evaluates both the credit-limit and the overdue conditions only when `blokirOtomatis` is set, where the real API applies the credit-limit condition regardless of that flag. Nor does the mock persist that a trip has already dispatched — it has no trips table to check against, so calling `dispatchTrip` twice for the same run succeeds twice on the mock build, unlike the real API's "already left" refusal. |
...
| outletwarnings | mock + http | new feature (D3 A-Step 3) — no prior mock-only row, same shape `transportation` was in. `core.outlet_warnings` backs a plain Surat Peringatan (SP) log end to end: read+create only, no update or delete — a warning is corrected by issuing a new entry, not editing one already handed to the outlet. No gaps between builds; the whole feature is three fields (`issuedOn`, `reason`, `notes`) and both adapters produce all three. |
...
| transportation | mock + http | new feature (D3 A-Step 1) — no prior mock-only row. `core.transportation_claims`/`core.transportation_claim_deliveries` back a BAST/transportation-fee claim end to end: system-numbered (`BAST-…`, gap-free across rollbacks), status-tracked through the fixed `draft→submitted→under_review→approved→rejected→paid` set, draft-only delete. The console labels the domain "BAST" and "iVendor" throughout even though the schema's own column names stay business-neutral (`handover_reference`, `principal_invoice_number`) per the D3 plan's decision 7. Delivery-linking UI is deliberately not built in this pass — the API accepts `delivery_ids` at creation, but a picker for selecting which deliveries a claim covers is out of scope for the smallest useful version; a claim can be recorded and tracked with zero deliveries attached. |
