...
import type { DateRange, MonitoringApi, SetDeliveryStatusInput } from "./contract";
import type {
  DriverCard,
...
async function getMonitoringSnapshot(dateRange: DateRange): Promise<MonitoringSnapshot> {
  ...
  // A stop's position in its own driver's day — the same thing sequence_no
  // means against the real service. Computed here rather than read off `d`
  // directly so both builds number a round the same way: per driver, in the
  // order it is actually driven.
  const urutanOf = new Map<string, number>();
  db.drivers.forEach((driver) => {
    drops
      .filter((d) => d.driverId === driver.id)
      .sort((a, b) => a.jamRencana.localeCompare(b.jamRencana))
      .forEach((d, i) => urutanOf.set(d.id, i + 1));
  });
...
async function setDeliveryStatus(input: SetDeliveryStatusInput) {
  await latency("write");
  const result = updateDeliveryStatus(input.deliveryId, input.status, input.realisasi, {
    catatan: input.catatan,
  });
  return { kode: result.kode, status: result.status, realisasi: result.realisasi };
}

/** Prints the surat jalan carried with the load. */
async function printSuratJalan(deliveryId: string): Promise<void> {
  await latency("read");
  const db = scopedDb();
...
export const monitoringApiMock: MonitoringApi = {
