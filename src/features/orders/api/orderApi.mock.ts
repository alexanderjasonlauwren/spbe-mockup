  ...
  return scopedDb()
    ...
    .filter((o) => {
      ...
      if (filters?.search) {
        ...
        return o.kode.toLowerCase().includes(q) || o.outlet.toLowerCase().includes(q);
      }
      return true;
    })
...
async function approveOrder(id: string, catatan?: string): Promise<OrderView> {
  await latency("write");
...
async function createOrder(input: {
  ...
  return mutate((db) => {
    ...
    const order: OrderEntity = {
      // Falls back to the ACTING tenant, not to "the" tenant: with a hierarchy
      // there is no single one, and stamping a row with the root's id while
      // acting as a subsidiary is a cross-tenant write the backend's WITH
      // CHECK would refuse.
...
export const orderApiMock: OrderApi = {
  getOrders,
  getOrderTotals,
  approveOrder,
  declineOrder,
