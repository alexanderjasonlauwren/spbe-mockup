/**
 * Outlets against the browser store.
 *
 * The demo build's implementation. It keeps every figure it always had —
 * deliveries and invoices are right there in the store — which is precisely
 * what the HTTP adapter cannot do, and why `statistikTersedia` exists.
 */
...
import type { OutletEntity } from "@/mocks/types";
...
function toView(p: OutletEntity): OutletView {
  ...
  return {
    ...
    // The store holds the deliveries and the invoices, so every figure above
    // is real here.
...
async function createOrUpdateOutlet(input: Partial<OutletEntity> & { id?: string }) {
  await latency("write");
...
export const outletApiMock: OutletApi = {
  getOutletList,
  getOutletDetail,
