...
async function saveSupplier(input: Partial<SupplierEntity> & { id?: string }) {
  await latency("write");
  return mutate((db) => {
...
async function saveNumbering(penomoran: NumberingEntity) {
  await latency("write");
...
async function saveOperations(operasi: OperationsEntity) {
  await latency("write");
...
export const systemApiMock: SystemApi = {
  getSupplierList,
  saveSupplier,
  deleteSupplier,
  getSystemConfig,
