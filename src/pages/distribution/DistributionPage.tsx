...
const mockDrivers: DriverAssignment[] = [
  {
    ...
    noKendaraan: "B 1234 AB",
    kapasitas: 150,
    pangkalanTujuan: ["UD Maju Jaya", "Kios Makmur"],
    jumlahTabung: 120,
    status: "on_trip",
  },
  {
...
export function DistributionPage() {
  ...
  const [allocationSortKey, setAllocationSortKey] = useState<
    "nama" | "targetBulanan" | "alokasi" | "driverAssigned" | "status"
  >("nama");
  const [allocationSortDirection, setAllocationSortDirection] =
    useState<SortDirection>("asc");
  const [realisasiSortKey, setRealisasiSortKey] = useState<
    | "noSJ"
    | "pangkalan"
    | "driver"
    | "jumlah"
    ...
    | "tanggal"
  >("noSJ");
  const [realisasiSortDirection, setRealisasiSortDirection] =
    useState<SortDirection>("desc");
  const [driverSortKey, setDriverSortKey] = useState<
    "nama" | "noKendaraan" | "kapasitas" | "jumlahTabung" | "status"
  >("nama");
  ...
  const belumTerjadwal = mockPangkalan.filter(
    (p) => p.status === "belum",
  ).length;
  ...
  const handleAllocationSort = (
    ...
    if (allocationSortKey === nextSortKey) {
  ...
  const handleRealisasiSort = (
    ...
    if (realisasiSortKey === nextSortKey) {
      setRealisasiSortDirection((prev) => (prev === "asc" ? "desc" : "asc"));
      return;
    }

    setRealisasiSortKey(nextSortKey);
    setRealisasiSortDirection("asc");
  };
  ...
  const sortedPangkalan = [...mockPangkalan]
    .filter((p) => p.nama.toLowerCase().includes(searchQuery.toLowerCase()))
    .sort((left, right) => {
      let compareValue = 0;

      if (allocationSortKey === "nama") {
        compareValue = left.nama.localeCompare(right.nama, "id-ID");
      }

      if (allocationSortKey === "targetBulanan") {
        compareValue = left.targetBulanan - right.targetBulanan;
      }

      if (allocationSortKey === "alokasi") {
        compareValue = left.alokasi - right.alokasi;
      }

      if (allocationSortKey === "driverAssigned") {
        compareValue = (left.driverAssigned || "-").localeCompare(
          right.driverAssigned || "-",
          "id-ID",
        );
      }

      if (allocationSortKey === "status") {
        compareValue = left.status.localeCompare(right.status, "id-ID");
      }

      return allocationSortDirection === "asc" ? compareValue : -compareValue;
    });

  const sortedRealisasi = [...mockRealisasi].sort((left, right) => {
    let compareValue = 0;

    if (realisasiSortKey === "noSJ") {
      compareValue = left.noSJ.localeCompare(right.noSJ, "id-ID");
    }

    if (realisasiSortKey === "pangkalan") {
      compareValue = left.pangkalan.localeCompare(right.pangkalan, "id-ID");
    }

    if (realisasiSortKey === "driver") {
      compareValue = left.driver.localeCompare(right.driver, "id-ID");
    }

    if (realisasiSortKey === "jumlah") {
      compareValue = left.jumlah - right.jumlah;
    }

    if (realisasiSortKey === "berangkat") {
      compareValue = left.berangkat.localeCompare(right.berangkat, "id-ID");
    }

    if (realisasiSortKey === "status") {
      compareValue = left.status.localeCompare(right.status, "id-ID");
    }

    if (realisasiSortKey === "tanggal") {
      compareValue =
        new Date(left.tanggal).getTime() - new Date(right.tanggal).getTime();
    }

    return realisasiSortDirection === "asc" ? compareValue : -compareValue;
  });

  const sortedDrivers = [...mockDrivers].sort((left, right) => {
    let compareValue = 0;
    ...
    if (driverSortKey === "jumlahTabung") {
      compareValue = left.jumlahTabung - right.jumlahTabung;
    }
    ...
    <div className="space-y-6">
      ...
      <Card className="border border-gray-200 dark:border-dark-700 bg-white dark:bg-dark-800 shadow-xl">
        <CardHeader className="p-0 border-b border-gray-100 dark:border-dark-700 bg-gray-50 dark:bg-dark-850">
          <div className="px-6 py-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="w-full lg:w-auto overflow-x-auto">
              <div className="inline-flex min-w-max border border-gray-200 dark:border-dark-700 rounded-xl p-1 bg-white dark:bg-dark-900 gap-1">
                {tabs.map((tab) => (
                  <button
                    key={tab.key}
        ...
        <CardContent className="p-0">
          ...
          {activeTab === "rencana" && (
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 border-b border-gray-100 dark:border-dark-700 bg-gray-50 dark:bg-dark-850">
                <div className="relative w-full sm:w-56">
                  ...
                  <Input
                    ...
                    className="pl-9 w-full bg-white dark:bg-dark-900 border-gray-300 dark:border-dark-600 text-sm"
                  />
                </div>
                        ...
                        >
                          ...
                          <TableCell className="px-4 py-3 text-center">
                            <Button
                              size="sm"
                              variant="outline"
                              className="text-xs h-7 border-cyan-300 dark:border-cyan-500/30 text-cyan-700 dark:text-cyan-400 hover:bg-cyan-50 dark:hover:bg-cyan-500/10"
                            >
                              Buat SJ
                            </Button>
                          </TableCell>
                ...
                <TableHeader>
                  <TableRow className="bg-gray-50 dark:bg-dark-850 hover:bg-gray-50 dark:hover:bg-dark-850 border-b border-gray-200 dark:border-dark-700">
                    <SortableTableHead
                      ...
                      sortKey="noSJ"
                      activeSortKey={realisasiSortKey}
                      sortDirection={realisasiSortDirection}
                      onSort={handleRealisasiSort}
                    />
                    <SortableTableHead
                      label="Pangkalan"
                      sortKey="pangkalan"
                            ...
                            <div className="flex flex-wrap gap-1">
                              {driver.pangkalanTujuan.map((p) => (
                                <Badge
                                  key={p}
                                  variant="outline"
                                  className="text-xs border-indigo-300 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10"
                                >
                                  {p}
                                </Badge>
                              ))}
                        ...
                        <TableCell className="px-4 py-3 text-right">
                          <div>
                            <span
                              className={cn(
                                "text-sm font-bold",
                                driver.jumlahTabung > driver.kapasitas
                                  ? "text-red-500"
                            ...
                            <div className="w-full bg-gray-200 dark:bg-dark-700 rounded-full h-1 mt-1">
                              <div
                                ...
                                style={{
                                  width: `${Math.min((driver.jumlahTabung / driver.kapasitas) * 100, 100)}%`,
                                }}
                              />
                            </div>
                          </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: "Pangkalan Terjadwal",
                  value: `${mockPangkalan.filter((p) => p.status !== "belum").length} dari ${mockPangkalan.length}`,
                  color: "text-green-600 dark:text-green-400",
                },
                {
                  label: "Driver Ditugaskan",
                  value: `${mockDrivers.filter((d) => d.status !== "available").length} dari ${mockDrivers.length}`,
                ...
                <div
                  key={item.label}
                  className="text-center p-3 rounded-xl bg-gray-50 dark:bg-dark-700/50 border border-gray-200 dark:border-dark-700"
                >
