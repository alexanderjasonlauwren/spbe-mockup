...
const mockPangkalan: Pangkalan[] = [
  { id: "1", nama: "UD Maju Jaya", alamat: "Jl. Raya Bogor No. 12", kota: "Jakarta Timur", noHP: "0812-3333-4444", noNPWP: "12.345.678.9-001.000", targetBulanan: 1800, distribusiBulanIni: 1640, pembayaranStatus: "lunas", status: "aktif", bergabungSejak: "Jan 2022", cabang: "Pusat - Jakarta" },
  { id: "2", nama: "Toko Berkah", alamat: "Jl. Sudirman No. 45", kota: "Jakarta Pusat", noHP: "0813-5555-6666", noNPWP: "23.456.789.0-002.000", targetBulanan: 1200, distribusiBulanIni: 1100, pembayaranStatus: "pending", status: "aktif", bergabungSejak: "Mar 2022", cabang: "Pusat - Jakarta" },
  { id: "3", nama: "CV Sejahtera", alamat: "Jl. Gatot Subroto No. 7", kota: "Jakarta Selatan", noHP: "0811-7777-8888", noNPWP: "34.567.890.1-003.000", targetBulanan: 1500, distribusiBulanIni: 1420, pembayaranStatus: "lunas", status: "aktif", bergabungSejak: "Nov 2021", cabang: "Pusat - Jakarta" },
  { id: "4", nama: "Kios Makmur", alamat: "Jl. Pahlawan No. 23", kota: "Depok", noHP: "0814-9999-0000", noNPWP: "45.678.901.2-004.000", targetBulanan: 900, distribusiBulanIni: 760, pembayaranStatus: "tunggakan", status: "aktif", bergabungSejak: "Jun 2023", cabang: "Cabang Depok" },
  { id: "5", nama: "UD Harapan", alamat: "Jl. Merdeka No. 88", kota: "Jakarta Barat", noHP: "0815-1111-2222", noNPWP: "56.789.012.3-005.000", targetBulanan: 1100, distribusiBulanIni: 980, pembayaranStatus: "lunas", status: "aktif", bergabungSejak: "Aug 2022", cabang: "Pusat - Jakarta" },
  { id: "6", nama: "Toko Sumber Rejeki", alamat: "Jl. Ahmad Yani No. 15", kota: "Bogor", noHP: "0816-3333-4444", noNPWP: "67.890.123.4-006.000", targetBulanan: 800, distribusiBulanIni: 0, pembayaranStatus: "lunas", status: "nonaktif", bergabungSejak: "Oct 2023", cabang: "Cabang Bogor" },
];

const bayarStatusConfig = {
  lunas: { label: "Lunas", className: "bg-green-100 text-green-700 border-green-300 dark:bg-green-500/10 dark:text-green-400 dark:border-green-500/30", icon: CheckCircle2 },
  pending: { label: "Pending", className: "bg-yellow-100 text-yellow-700 border-yellow-300 dark:bg-yellow-500/10 dark:text-yellow-400 dark:border-yellow-500/30", icon: AlertCircle },
  tunggakan: { label: "Tunggakan", className: "bg-red-100 text-red-700 border-red-300 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/30", icon: AlertCircle },
};
...
export function ProductListPage() {
  ...
  const filtered = mockPangkalan.filter((p) => {
    ...
    const matchesBayar = bayarFilter === "all" || p.pembayaranStatus === bayarFilter;
  ...
  const totalDistribusi = mockPangkalan.reduce((s, p) => s + p.distribusiBulanIni, 0);
  const tunggakan = mockPangkalan.filter((p) => p.pembayaranStatus === "tunggakan").length;
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-dark-850 hover:bg-gray-50 dark:hover:bg-dark-850 border-b border-gray-200 dark:border-dark-700">
                  ...
                  <TableHead className="px-4 py-3 text-xs font-semibold uppercase text-gray-700 dark:text-gray-300 min-w-[160px]">Distribusi Bulan Ini</TableHead>
                  <TableHead className="px-4 py-3 text-xs font-semibold uppercase text-gray-700 dark:text-gray-300">Pembayaran</TableHead>
              ...
              <TableBody>
                {filtered.map((p) => {
                  ...
                  const distribusiPct = Math.round((p.distribusiBulanIni / p.targetBulanan) * 100);

                  return (
                    <TableRow key={p.id} className="border-b border-gray-100 dark:border-dark-700 hover:bg-indigo-50/30 dark:hover:bg-indigo-500/5 transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1 mt-0.5">
                              ...
                              <p className="text-xs text-gray-500 dark:text-gray-400">{p.noHP}</p>
                            </div>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-3">
                        <p className="text-sm text-gray-900 dark:text-white">{p.kota}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{p.cabang}</p>
                      </TableCell>
                      <TableCell className="px-4 py-3 text-right">
                        <p className="font-bold text-gray-900 dark:text-white">{p.targetBulanan.toLocaleString("id-ID")}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">tabung/bln</p>
                      </TableCell>
                      <TableCell className="px-4 py-3">
                        <div className="space-y-1">
                          <div className="flex justify-between text-xs">
                            <span className="font-medium text-gray-900 dark:text-white">{p.distribusiBulanIni.toLocaleString("id-ID")}</span>
                            <span className={cn("font-semibold", distribusiPct >= 90 ? "text-green-600 dark:text-green-400" : distribusiPct >= 70 ? "text-yellow-600 dark:text-yellow-400" : "text-red-600 dark:text-red-400")}>
                              {distribusiPct}%
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 dark:bg-dark-700 rounded-full h-1.5">
                            <div
                              className={cn("h-1.5 rounded-full", distribusiPct >= 90 ? "bg-green-500" : distribusiPct >= 70 ? "bg-yellow-500" : "bg-red-500")}
                              style={{ width: `${Math.min(distribusiPct, 100)}%` }}
                            />
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-3">
                        <Badge variant="outline" className={cn("flex items-center gap-1 w-fit text-xs font-medium", bayarConf.className)}>
                      ...
                      <TableCell className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <CanAccess permission={PERMISSIONS.PRODUCTS_EDIT}>
                            <Button size="sm" variant="outline" className="text-xs h-7 border-indigo-300 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-400 hover:bg-indigo-50">
                              Edit
                            </Button>
                          </CanAccess>
                        </div>
